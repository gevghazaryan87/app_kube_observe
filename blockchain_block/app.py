...
def index():
    try:
        with tracer.start_as_current_span("flask.db.fetch_blocks"):
...
def block_details(block_hash):
    try:
        with tracer.start_as_current_span("flask.db.fetch_block_details", attributes={"bitcoin.block.hash": block_hash}):
            conn = get_db_connection()
            cur = conn.cursor(cursor_factory=RealDictCursor)
...
def transaction_details(txid):
    ...
    try:
        with tracer.start_as_current_span("flask.db.fetch_tx_details", attributes={"bitcoin.txid": txid}):
            conn = get_db_connection()
            cur = conn.cursor(cursor_factory=RealDictCursor)
...
if __name__ == '__main__':
    ...
    # use_reloader=False is required for OpenTelemetry to work correctly in Docker
